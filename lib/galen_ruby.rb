...
class Array
  def print_columns(include_ints=false)
    index_len = self.count.to_s.length
    things_to_print = []
    self.each_with_index do |val, i|
      things_to_print << (include_ints ? "#{i.to_s.rjust(index_len)}) #{val}" : val.dup)
    end

    max_len = things_to_print.sort_by{ |val| val.length }[-1].length

    column_count = (1..(TERM_WIDTH / max_len)).to_a.select{ |i| ((max_len * i) + (5 * (i - 1))) < TERM_WIDTH }[-1]
    line_count = (self.count.to_f / column_count).ceil

    things_to_print.each_with_index.to_a.group_by{ |val, i| i % line_count }.each do |line_number, things|
      puts things.map{ |x, i| x.ljust(max_len) }.join('  |  ')
    end
  end

  # these are for managing arrays of hashes, e.g. CSVs with headers or relatively flat JSON files
  def headers
    self.map{ |r| r.keys }.flatten.uniq
  end

  def swap(current_header_order, new_headers)
    header_swapper = current_header_order.count.times.to_a.map{ |i| [current_header_order[i], new_headers[i]] }.to_h
    new_version = []
    self.each do |r|
      new_version << r.map{ |k, v| [header_swapper[k], v] }.to_h
    end
...
class Hash
  def no_empties
    self.select{ |k, v| v.to_s.strip.length > 0 }
  end
end
